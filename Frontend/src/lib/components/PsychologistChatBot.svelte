<script lang="ts">
  ...
  import { 
    psychologistChatStore, 
    isLoadingPsychologistStore, 
    initPsychologistChat, 
    sendMessageToPsychologist, 
    clearPsychologistChat
  } from '$lib/services/psychologist';
  ...
  isLoadingPsychologistStore.subscribe(value => {
    isLoading = value;
  });
  ...
  function toggleChatDialog() {
    ...
    if (showChatDialog && $psychologistChatStore.length === 0) {
      initPsychologistChat();
    }
  }
  ...
  async function handleSendMessage() {
    if (userMessage.trim() !== '') {
      await sendMessageToPsychologist(userMessage);
      userMessage = '';
    }
  }
  ...
  onMount(() => {
    // Если уже показываем диалог, инициализируем чат с психологом
    if (showChatDialog && $psychologistChatStore.length === 0) {
      initPsychologistChat();
    }
  });
  ...
  <div class="chat-bot">
    {#if showChatBubble}
      <div class="chat-bubble">
        ...
        Привет! Я - ваш виртуальный психолог.<br>Готов вас выслушать и помочь
      </div>
    {/if}
    <div class="bot-icon" on:click={toggleChatDialog}>
      <img src="/zigmund.png" alt="Психолог" onerror="this.src='/main_page/happy emoji with hearts.png'" />
    </div>
  </div>
      ...
      <div class="chat-dialog">
        <div class="chat-header">
          <div class="chat-header-left">
            <img src="/zigmund.png" alt="Психолог" class="chat-avatar" onerror="this.src='/main_page/happy emoji with hearts.png'" />
            <h3>Виртуальный психолог</h3>
          </div>
        ...
        <div class="chat-messages">
          {#each $psychologistChatStore as message}
            <div class="message {message.sender === 'Psychologist' ? 'bot' : 'user'}">
              <div class="message-sender">{message.sender === 'Psychologist' ? 'Психолог' : 'Вы'}</div>
              <div class="message-content">
                {#if message.sender === 'Psychologist'}
